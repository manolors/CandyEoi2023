...
async function getData() {
  ...
  json.forEach((product) => {
    if (product.id == idProduct) {
      ...
      $div.innerHTML = `<div class="w-full flex flex-col md:flex-row items-center border-blue-800 flex-row max-w-xl gap-5">
                          ...
                          <img class="w-56 md:w-auto" src="${product.image}" alt="${product.name}" />
                          </div>
                          ...
                          <img class="w-8 md:w-10" src="assets/icon/leche.jpeg" alt="Lactosa"></img>
                          <img class="w-12 md:w-14" src="assets/icon/azucar.webp" alt="Azúcar"></img>
                          </div>
                          </div>
                          </div>
                          <div>
  ...
  buy.addEventListener("click", () => {
    addProduct(buy.dataset.value, buy.dataset.image, buy.dataset.name, buy.dataset.price);
...
async function getDataRecommended() {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    $div.innerHTML = `<div class="flex flex-col md:flex-col lg:flex-row lg:gap-9 md:gap-3 gap-3 md:w-48 md:h-48 lg:w-full lg:h-72">
                        ...
                        <img class="w-20 h-20 md:w-full lg:w-full md:h-full lg:h-48 rounded-l-lg md:rounded-t-lg md:rounded-bl-none lg:rounded-t-lg lg:rounded-bl-none" src="${json[i].image}" alt="${json[i].name}"></a>
                        <div class="flex flex-col lg:p-4 gap-2">
                        <h1><a class="font-bold text-left p-3" href="detalle.html?id=${json[i].id}">${json[i].name}</a></h1>
                        <h3 class="font-semibold">${json[i].price}€</h3>
                        </div>
                        </div>
                        </div>`;
